...
export class AuthenticationController extends BaseController {
  createJwtFromUser = (user: HydratedDocument<User>): [Token, Token] => {
  ...
  authenticateUser = (user: HydratedDocument<User>, res: Response) => {
    const [access_token, refresh_token] = this.createJwtFromUser(user);
  ...
  login = async (req: Request, res: Response, next: NextFunction) => {
    ...
    return res.status(200).json(this.authenticateUser(user, res));
  };
  ...
  oauthLogin = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (existingUser) {
        return res.status(200).json(this.authenticateUser(existingUser, res));
      } else {
        ...
        return res.status(201).json(this.authenticateUser(createdUser, res));
      }
    } catch (e) {
  ...
  refreshToken = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const refreshTokenValue =
      tokenFromCookie || tokenFromBody || authHeaderToken;
    ...
    jwt.verify(
      ...
      async (err, val) => {
        ...
        return res.json(this.authenticateUser(user, res));
      },
    );
  };
}
